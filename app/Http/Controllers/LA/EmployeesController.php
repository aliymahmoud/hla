...
class EmployeesController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Employees", "create")) {
			...
			// generate password
			$password = '123456';//LAHelper::gen_password();
	...
	public function dtajax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Employees", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/employees/'.$data->data[$i]->id.'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
