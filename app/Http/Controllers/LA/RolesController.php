...
class RolesController extends Controller
{
	...
	public $listing_cols = ['id', 'name', 'parent'];
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Roles", "create")) {
			...
			$role->givePermissionTo("ADMIN_PANEL");
	...
	public function destroy($id)
	{
		if(Module::hasAccess("Roles", "delete")) {
			$message=$this->checkAvailabilityToDelete($id,array("Role"=>array("roles","parent")));
			$role=Role::find($id);
			if(count($role->users)>0)
			{
				if($message)
					$message ="couldn`t be deleted there is related (roles - users)";
				else
					$message = "couldn`t be deleted there is related (roles - users)";
			}
			if($message)
	...
	public function dtajax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				$col = $this->listing_cols[$j];
					...
					if($col == $this->view_col) {
						$data->data[$i]->$col = '<a href="'.url(config('laraadmin.adminRoute') . '/roles/'.$data->data[$i]->id).'">'.$data->data[$i]->$col.'</a>';
					}
					// else if($col == "author") {
					//    $data->data[$i]->$col;
					// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Roles", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/roles/'.$data->data[$i]->id.'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
