...
class UploadsController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function get_file(Request $request,$hash, $name,$thumb=0)
    {
        ...
        if($upload->public || Auth::user()->hasRole("SUPER_ADMIN") || Auth::user()->id == $upload->user_id) {
			...
			if($thumb==1) {
                ...
                $thumbpath = storage_path("thumbnails/".basename($upload->path));

                if(File::exists($thumbpath)) {
                    $path = $thumbpath;
                } else {
                    // Create Thumbnail
                    \App\Http\Helpers::resize($upload,360,210,1);
                    $path = $thumbpath;
                }
            }
            if($thumb==2) {
                
                $thumbpath = storage_path("thumbnails/thumb-".basename($upload->path));
                ...
                if(File::exists($thumbpath)) {
                    ...
                    \App\Http\Helpers::resize($upload,100,100,2);
                    $path = $thumbpath;
                }
            }
