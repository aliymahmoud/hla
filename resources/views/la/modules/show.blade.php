				...
				<table class="table table-bordered dataTable no-footer table-access">
					...
					@foreach($roles as $role)
						<tr class="tr-access-basic" role_id="{{ $role->id }}">
							...
							<td><input class="view_checkb" type="checkbox" name="module_view_{{$role->id}}" id="module_view_{{$role->id}}" <?php if($role->hasPermissionTo($module->name.'-view')) { echo 'checked="checked"'; } ?> ></td>
							<td><input class="create_checkb" type="checkbox" name="module_create_{{$role->id}}" id="module_create_{{$role->id}}" <?php if($role->hasPermissionTo($module->name.'-create')) { echo 'checked="checked"'; } ?> ></td>
							<td><input class="edit_checkb" type="checkbox" name="module_edit_{{$role->id}}" id="module_edit_{{$role->id}}" <?php if($role->hasPermissionTo($module->name.'-edit')) { echo 'checked="checked"'; } ?> ></td>
							<td><input class="delete_checkb" type="checkbox" name="module_delete_{{$role->id}}" id="module_delete_{{$role->id}}" <?php if($role->hasPermissionTo($module->name.'-delete')) { echo 'checked="checked"'; } ?> ></td>
							<td>
								...
								@foreach (array_chunk($module->fields, 3, true) as $fields)
									<tr>
										@foreach ($fields as $field)
											<td><div class="col-md-3"><input type="text" name="{{ $field['colname'] }}_{{ $role->id }}" value="{{ !empty($role->fields[$field['id']])?$access[$role->fields[$field['id']]]:0 }}" data-slider-value="{{ !empty($role->fields[$field['id']])?$access[$role->fields[$field['id']]]:0 }}" class="slider form-control" data-slider-min="0" data-slider-max="2" data-slider-step="1" data-slider-orientation="horizontal"  data-slider-id="{{ $field['colname'] }}_{{ $role->id }}"></div> {{ $field['label'] }} </td>
										@endforeach
									</tr>
								@endforeach
								</table>
