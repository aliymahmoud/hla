...
<div id="page-content" class="profile2">
	...
	<ul data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		...
		@role("SUPER_ADMIN")
		<li class=""><a role="tab" data-toggle="tab" href="#tab-access" data-target="#tab-access"><i class="fa fa-key"></i> Permissions</a></li>
		@endrole
	</ul>
	...
	<div class="tab-content">
			...
			<div class="tab-content">
				<div class="panel infolist">
					...
					<div class="panel-body">
						...
						{{-- @la_display($module, 'view_data') --}}
						{{-- @la_display($module, 'view_dept') --}}
						</div>
				</div>
			</div>
		</div>
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			<div class="guide1">
				<span class="pull-left">Module Accesses for {{ $role->name }} Role</span>
			...
			<form action="{{ url(config('laraadmin.adminRoute') . '/save_module_role_permissions/'.$role->id) }}" method="post">
				...
				<table class="table table-bordered dataTable no-footer table-access">
					...
					@foreach($modules_access as $mmodule)
						<tr>
							<td><input module_id="{{ $mmodule->id }}" class="module_checkb" type="checkbox" name="module_{{$mmodule->id}}" id="module_{{$mmodule->id}}" checked="checked">&nbsp; {{ $mmodule->name }}</td>
							<td><input module_id="{{ $mmodule->id }}" class="view_checkb" type="checkbox" name="module_view_{{$mmodule->id}}" id="module_view_{{$mmodule->id}}" <?php if($mmodule->accesses->view == 1) { echo 'checked="checked"'; } ?> ></td>
							<td><input module_id="{{ $mmodule->id }}" class="create_checkb" type="checkbox" name="module_create_{{$mmodule->id}}" id="module_create_{{$mmodule->id}}" <?php if($mmodule->accesses->create == 1) { echo 'checked="checked"'; } ?> ></td>
							<td><input module_id="{{ $mmodule->id }}" class="edit_checkb" type="checkbox" name="module_edit_{{$mmodule->id}}" id="module_edit_{{$mmodule->id}}" <?php if($mmodule->accesses->edit == 1) { echo 'checked="checked"'; } ?> ></td>
							<td><input module_id="{{ $mmodule->id }}" class="delete_checkb" type="checkbox" name="module_delete_{{$mmodule->id}}" id="module_delete_{{$mmodule->id}}" <?php if($mmodule->accesses->delete == 1) { echo 'checked="checked"'; } ?> ></td>
							<td>
								<a module_id="{{ $mmodule->id }}" class="toggle-adv-access btn btn-default btn-sm hide_row"><i class="fa fa-chevron-down"></i></a>
							</td>
						</tr>
						<tr class="tr-access-adv module_fields_{{ $mmodule->id }} hide" module_id="{{ $mmodule->id }}" >
							<td colspan=6>
								<table class="table table-bordered">
								@foreach (array_chunk($mmodule->accesses->fields, 3, true) as $fields)
									<tr>
										@foreach ($fields as $field)
											<td><div class="col-md-3"><input type="text" name="{{ $field['colname'] }}_{{ $mmodule->id }}_{{ $role->id }}" value="{{ $field['access'] }}" data-slider-value="{{ $field['access'] }}" class="slider form-control" data-slider-min="0" data-slider-max="2" data-slider-step="1" data-slider-orientation="horizontal"  data-slider-id="{{ $field['colname'] }}_{{ $mmodule->id }}_{{ $role->id }}"></div> {{ $field['label'] }} </td>
										@endforeach
									</tr>
								@endforeach
								</table>
