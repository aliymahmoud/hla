...
class LAHelper
{
	...
	public static function print_menu($menu, $active = false,$module=null) {
		$ch = \Dwij\Laraadmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();
                $childrens=array();
                foreach($ch as $c)
                {
					if($c->name)
                    {
						$temp_module_obj = Module::get($c->name);
						if(!empty($temp_module_obj))
						{
							if(Module::hasAccess($temp_module_obj->id))
							{
								$childrens[]=$c;
								if(isset($module->id) && strtolower($module->name) == strtolower($c->name))
								{
									$active=true;
								}
							}
						...
						{
							$childrens[]=$c;
							if(url()->current()==url(config('laraadmin.adminRoute'))."/".$c->url)
								$active=true;
						}
                    }
                    else
		...
		$active_str = '';
                if($active || (isset($module->id) && strtolower($module->name) == strtolower($menu->name)) || url()->current()==url(config('laraadmin.adminRoute'))."/".$menu->url) {
			
                    $active_str = ' active'; 
		}
                $str ="";
		if(count($childrens) || count($ch)==0) 
		$str = '<li class="'.$treeview.' '.$active_str.'"><a href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'"><i class="fa '.$menu->icon.'"></i> <span>'.LAHelper::real_module_name($menu->label).'</span> '.$subviewSign.'</a>';
		if(count($childrens)) {
			$str .= '<ul class="treeview-menu">';
			foreach($childrens as $children) {
				$str .= LAHelper::print_menu($children,false,$module);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	public static function print_menu_topnav($menu, $active = false) {
		...
		$str = '<li '.$treeview.''.$active_str.'><a '.$treeview2.' href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'">'.LAHelper::real_module_name($menu->label).$subviewSign.'</a>';
