				...
				case 'Dropdown':
					...
					if($popup_vals != "") {
						...
						$popup_vals = array();
                                        ...
                                        if(!$required)
                                        {
                                            $popup_vals=[null=>'please select']+$popup_vals;
                                        }
...
//					dd($popup_vals);
				...
				case 'File':
					...
					if(isset($upload->id)) {
						$out .= "<a class='btn btn-default btn_upload_file hide' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
                                                    <input type='file' id='".$field_name."_upload_files' style='display:none' name='".$field_name."_input' />
							<a class='uploaded_file' target='_blank' href='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."'><i class='fa fa-file-o'></i><i title='Remove File' class='fa fa-times'></i></a>";
                                                ...
                                                $out .= "<input type='file' id='".$field_name."_upload_files' style='display:none' name='".$field_name."_input' />";
				...
				case 'Files':
					...
					$out .= "<input type='file' id='".$field_name."_upload_files' style='display:none' name='".$field_name."_input[]' multiple />";
				...
				case 'Image':
					...
					$req_str=!empty($required)? "required":"";
					if(isset($upload->id)) {
						$out .= "<input type='file' ".$req_str." id='".$field_name."_upload_files' style='display:none' name='".$field_name."_input' />"
                                                        . "<a class='btn btn-default btn_upload_image hide' file_type='image' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<div class='uploaded_image'><img src='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."' width='150'><i title='Remove Image' class='fa fa-times'></i></div>";
					} else {
                                            $out .= "<input type='file' ".$req_str." id='".$field_name."_upload_files' style='display:none' name='".$field_name."_input' />";
				...
				case 'Taginput':
					...
					$params['multiple'] = "true";
					$params['rel'] = "taginput";
					$params['data-placeholder'] = "Add multiple ".$label;
					unset($params['placeholder']);
...
//					$out .= Form::select($field_name."[]", $popup_vals, $default_val, $params);
                ...
                case 'TextEditor':
                    ...
                    $out .= '<textarea class=" form-control ckeditor" name="'.$field_name.'">'.$default_val.'</textarea>';
//                        Form::textarea($field_name, $default_val, $params);
