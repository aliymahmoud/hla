...
class Module extends Model
{
	...
	public static function create_field_schema($table, $field, $update = false, $isFieldTypeChange = false) {
		...
		switch ($field->field_type) {
			...
			case 'Date':
				...
				if($field->defaultvalue != "" && !Str::startsWith($field->defaultvalue, "date")) {
					$var->default($field->defaultvalue);
				} else if($field->required) {
					$var->default("2020-01-01");
				}
				break;
			case 'Datetime':
				if($update) {
				...
				if(isset($var) && $field->defaultvalue != "" && !Str::startsWith($field->defaultvalue, "date")) {
					$var->default($field->defaultvalue);
				} else if($field->required) {
					$var->default("2020-01-01 00:00:00");
				}
				break;
			case 'Decimal':
				$var = null;
	...
	**/
	public static function getDDArray($module_name) {
		...
		if(isset($module)) {
			...
			if($model_name == "Role" || $model_name == "Permission") {
				$model = "Spatie\\Permission\\Models\\".ucfirst(Str::singular($module_name));
			} else {
	...
	public static function processDBRow($module, $request, $row) {
		...
		foreach ($module->fields as $field) {
			if(isset($request->{$field['colname']}) || isset($request->{$field['colname']."_hidden"})) {
				...
				if(empty($row->{$field['colname']} ) && !empty($field['defaultvalue']))
	...
	public static function itemCount($module_name) {
		...
		if(isset($module)) {
			...
			if($model_name == "Role" || $model_name == "Permission") {
				$model = "Spatie\\Permission\\Models\\".ucfirst(Str::singular($module_name));
				if(file_exists(base_path('vendor/spatie/laravel-permission/src/Models/'.$model_name.".php"))) {
					$model = "Spatie\\Permission\\Models\\".$model_name;
					return $model::count();
				} else {
	...
	public static function getRoleAccess($module_id, $specific_role = 0) {
		...
		foreach ($roles_arr as &$role) {
			...
			//if(isset($module_perm->id)) {
				// set db values
				$role->view = $role->hasPermissionTo($module->name."-view");//$module_perm->acc_view;
				$role->create = $role->hasPermissionTo($module->name."-create");//$module_perm->acc_create;
				$role->edit = $role->hasPermissionTo($module->name."-edit");//$module_perm->acc_edit;
				$role->delete = $role->hasPermissionTo($module->name."-delete");//$module_perm->acc_delete;
			/*} else {
				$role->view = false;
				$role->create = false;
				$role->edit = false;
			...
			foreach ($module->fields as $field) {
				...
				//$fields[$field['id']] = $field_perm->access;
			}
			$role->fields=$fields;
			$roles[] = $role;
		}
	...
	**/
	public static function hasAccess($module_id, $access_type = "view", $user_id = 0) {
		...
		if($user_id) {
			...
			if(isset($user->id)) {
				return $user->can($module->name.'-'.$access_type);
			}
		} else {
			//$roles = \Auth::user()->roles();
			return auth()->user()->can($module->name.'-'.$access_type);
		}
		/*
	...
	**/
	public static function hasFieldAccess($module_id, $field_id, $access_type = "view", $user_id = 0) {
		...
		foreach ($roles->get() as $role) {
			//$module_perm = DB::table('role_module')->where('role_id', $role->id)->where('module_id', $module_id)->first();
			//if(isset($module_perm->id)) {
				if($access_type == "view" && $role->hasPermissionTo($module->name.'-'.$access_type)) {
					$hasModuleAccess = true;
					break;
				} else if($access_type == "write" && ($role->hasPermissionTo($module->name.'-create')|| $role->hasPermissionTo($module->name.'-edit'))) {
					$hasModuleAccess = true;
					break;
				} else {
					continue;
	...
	**/
	public static function setDefaultRoleAccess($module_id, $role_id, $access_type = "readonly") {
		...
		// 1. Set Module Access
		if($access_view)
			$role->givePermissionTo($module->name.'-view');
		else
			$role->revokePermissionTo($module->name.'-view');
		if($access_create)
			$role->givePermissionTo($module->name.'-create');
		else
			$role->revokePermissionTo($module->name.'-create');
		if($access_edit)
			$role->givePermissionTo($module->name.'-edit');
		else
			$role->revokePermissionTo($module->name.'-edit');
		if($access_delete)
			$role->givePermissionTo($module->name.'-delete');
		else
			$role->revokePermissionTo($module->name.'-delete');
		/*
		$module_perm = DB::table('role_module')->where('role_id', $role->id)->where('module_id', $module->id)->first();
		if(!isset($module_perm->id)) {
